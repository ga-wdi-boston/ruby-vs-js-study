...
```md
<!-- So the big differences I have noticed so far is, one it uses doesn't need a command to assign a variable. A second is its use of the = is different requring 2 for comparison and three having a completely different purpose. The final difference I have noticed would be its use of language and syntax, for example if statments are constructed very differently.  -->
```
...
const boomhauer = new Person('Jimmy normal');
