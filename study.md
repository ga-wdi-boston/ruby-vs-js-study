...
```md
No let or const. Constant variables are declared using a capital letter.

additional functionality on the +=, *=, /= ||= etc. vs only += and -= with JS

arguments go in |pipes| rather than (parens)
```
...
parseInt(left_operand) + parseInt(right_operand)
...
const filterNumbers = numbers.filter(num => num % 3 === 0)
```
