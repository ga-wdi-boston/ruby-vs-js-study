...
```md
<!-- Java script let/const variable decliration
    Ruby's really cool function chaining
    the auto return funciton in ruby, no need to speicfy a return function-->
```
...
const sum = (left_operand * 1) + (right_operand * 1)
```
...
const divByThree = (num) => {if(num % 3 === 0){return num}}
const divByThreeOnly = numbers.filter(divByThree)
// your answer here
console.log(divByThreeOnly)
```
...
squared_array = numbers.map { |num| num * num}
```
