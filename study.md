...
```md
Ruby has classes with methods where JavaScript has prototypes with methods,
Ruby syntax is modeled more on natural speech and is more intuitive compared to
JavaScript, and Ruby has a couple of ways as far as I know so far to set up blocks
of code in conditional statement and in functions. One can either use {} like in
Javascript or they can have a "do" operator at the beginning of the block
followed by an "end" operator at the end of the block. Semicolons are also not
not needed at the end of statements in Ruby as they are in JavaScript.
```
