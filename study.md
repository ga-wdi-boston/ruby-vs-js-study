...
```md
1. Everything in Ruby is an object as compared to Javascript where not everything is an object.
2. There are no functions only methods on objects
3. There is no use of curly brackets.
```
...
var divThree = function (num) {
return num % 3 === 0;
}

numbers.filter(divThree)
```
