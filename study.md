...
```md
<!-- Diff#1  The way variables are declared differ between JS and Ruby.
In Ruby, no keyword (let or const) is required and a value is required.  In
Javascript, let or const is required and a value is not required.

    Diff#2   JS and Ruby differ in the way functions are defined and in how
    they are called.
...
```ruby
str_into_array_reversed = str.reverse.split('')
str_joined = str_into_array_reversed.join('-')
...
const modThree = function (num) {
return num % 3 === 0
}

let filteredNumbers = numbers.filter(modThree)
...
squares = []
squares = arr.each { |squares| puts squares **2, " " }
