...
```md
No use of let and const in Ruby, just use the varable name.
Ruby has lots of short cuts to writing code.  Don't need () or {} making coding faster.
Looping methods in Ruby are much more flexible and written quickly.
=== vs ==
++ and -- are not in Ruby
```
...
str.reverse
str.split(//)
str.reverse.each_char{|c| print c, '-'}
```
...
const jeff = new Person('Jeff Boomhauer');
jeff.hello();
```
