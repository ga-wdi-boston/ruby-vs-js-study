...
```md
1. you cannot dot notations to access hash keys.
2. No need for semi-colons
3. No need for () when declaring if conditions.
```
...
first_num = "1"
second_num = "2"
sum = parseInt(first_num) + parseInt(second_num)
```
...
let filteredNumbers = filter((value)=>x%3===0)
```
