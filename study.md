...
RUBY - uses node tools
JavaScript - uses pry and irb tools

RUBY - uses do...end construct (Block)
JavaScript - uses { }

RUBY - uses methods/classes
def add(a, b)
  puts a+b
end
...
```javascript
const Person = function initialize (name) {
  this.name = name
}
...
const boomhauer = Person('Jeff Boomhauer')
boomhauer.hello()
```
...
const filtered_numbers = numbers.filter(function () {
  return (number % 3 === 0)
})
