...
```md
1.) In JavaScript, a keyword is needed in front of a declared variable.  In Ruby, a keyword is not needed.
2.) JavaScript uses camelCase.  Ruby uses snake_case.
3.) In Javascript, "else if" is used is used for conditionality.  In Ruby, it's "elsif".
```
...
const filtered_numbers = numbers.filter((number) => number % 3 === 0)
}
```
