...
```md
Hashes vs objects
Ruby only has 2 false values (nil, false), javascript has several(0, '', undefined, false, null)
Ruby has way more built-in methods
...
str = "eeffoc"
str.reverse!
str.chars.to_a or str.split("").to_a
str.join("-")
...
```javascript
var notThree = numbers.filter(function(number){
  return number % 3 !== 0
})
```
